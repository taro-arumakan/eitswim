...
<li class="active">2018 AW COLLECTION CITY</li>
...
if( function_exists('elc_breadcrumb') === false ):
  function elc_breadcrumb(){
    ...
    if(!elc_is_home()&&!is_admin()){ /* !is_admin は管理ページ以外という条件分岐 */
      ...
      $str.= '<li><a href="' . main_home_url('/') .'">HOME</a></li>'."\n";
      /* 投稿のページ */
      if(is_single()){
        if($post->post_type!=='post') {
          ...
          $str .= '<li><a href="'.main_home_url('/').$news_slug.'"> '.$news_title.' </a></li>' . "\n";
          /**
      ...
      elseif($is_news){
        $news_slug = get_option( 'elc_news_page_news_slug', 'news' );
        $news_title = get_option( 'elc_news_page_news_title', 'NEWS' );
        $str .= '<li class="active"><a href="'.main_home_url('/').$news_slug.'"> '.$news_title.' </a></li>' . "\n";
        //$news_title = get_option( 'elc_news_page_news_title', 'News' );
      ...
      elseif(is_category()) {
        ...
        $news_title = get_option( 'elc_news_page_news_title', 'COLLECTION' );
        $str .= '<li><a href="'.home_url('/').'"> '.$news_title.' </a></li>' . "\n";
        if($cat -> parent != 0){
        ...
        //$u = str_replace('/category/', '/', $u);
        $str.='<li class="active">'. $cat -> name .'</li>'."\n";
      }
