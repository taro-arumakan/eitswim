...
add_filter( 'thk_create_description', function( $pid = null, $len = 100 ) use( $luxe ) {
  global $is_news;
	...
	elseif( is_singular() === true || $is_news === true) { // 個別記事と固定ページ
    //description_enable(true)
    $description_enable = get_theme_mod( 'description_enable', true );
    if($is_news === true) {
      $news_desc = get_option( 'elc_news_page_news_description', '' );
    ...
    if($description_enable===true) {
      //カスタムフィールの値を設定
      global $wp_query;
      $postid = $wp_query->post->ID;
      $desc = get_post_meta($postid, 'elc_description', true);
      if($is_news === true) {
        $desc = $news_desc;
      }
    } else {
