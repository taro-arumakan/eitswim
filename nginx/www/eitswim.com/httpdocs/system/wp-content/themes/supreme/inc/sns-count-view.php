...
?>
<a href="<?php echo $permalink; ?>" target="_blank"><?php the_post_thumbnail( 'thumbnail', array() ); ?></a>
<?php
				}
				else {
?>
...
?>
<td class="post-date"><?php echo get_the_date(); ?></td>
<?php
				}
				...
				if( isset( $cache_content[$permalink] ) ) {
					foreach( $cache_content[$permalink] as $val ) {
						...
						if( ctype_digit( $val ) === true ) {
							echo $cnt, apply_filters( 'thk_sns_count', $val, $id, $permalink ), '</td>';
						}
						else {
							echo '<td class="alert-icon">!</td>';
						}
					...
					echo '<td colspan="5">' . __( 'Not cached', 'luxeritas' ) . '</td>';
				}
?>
</tr>
</tbody>
