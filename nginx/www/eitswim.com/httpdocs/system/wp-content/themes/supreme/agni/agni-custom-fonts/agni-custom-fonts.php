...
require_once AGNI . '/agni-custom-fonts/agni-custom-fonts-upload.php';

// add submenu page 
function agni_custom_fonts_submenu_page()
{
  /**
	add_submenu_page( 'cookie', esc_html__( 'Custom Fonts', 'cookie' ), esc_html__( 'Custom Fonts', 'cookie' ), 'edit_theme_options', 'cookie-custom-fonts', 'agni_custom_fonts' );
   */
  add_submenu_page( 'luxe', esc_html__( 'Custom Fonts', 'luxe' ), esc_html__( 'Custom Fonts', 'luxe' ), 'edit_theme_options', 'cookie-custom-fonts', 'agni_custom_fonts' );
}
...
function agni_custom_fonts(){
	...
	<div class="wrap">
		<h1><?php esc_html_e( 'Custom Webfonts', 'cookie' ); ?></h1>
		...
		?>
		<form action="admin.php?page=cookie-custom-fonts" method="post" enctype="multipart/form-data">
			<div>
				<h2><?php esc_html_e( 'Enter Font Name', 'cookie' ); ?></h2>
				<input type="text" name="text" required />
			</div><br />
			<div>
				<h3><?php esc_html_e( 'Upload Webfont Files', 'cookie' ); ?></h3>
				<p><?php esc_html_e( 'Tip. you can add multiple files at once .eot, .woff, .svg, .ttf, .otf supported.', 'cookie' ); ?></p>
		...
		<div>
			<h3><?php esc_html_e( 'Font List :', 'cookie' ); ?></h3>
			<p><?php esc_html_e( 'List of fonts in Custom Fonts directory.', 'cookie' ); ?></p>
		</div>
		
		<table class="widefat">
		<thead>
			<tr>
				<?php echo '<th>'.esc_html__( 'Font', 'cookie' ).'</th><th>'.esc_html__( 'Font Family', 'cookie' ).'</th><th>'.esc_html__( 'Manage', 'cookie' ).'</th>'; 
				?>
			</tr>
		</thead>
		<tfoot>
		...
		<?php if ($handle = opendir($file_dirname)) {
		    ...
		    while (false !== ($file = readdir($handle))) {
		        if (!in_array($file, $blacklist)) {
					...
					<form action="admin.php?page=cookie-custom-fonts" method="post" >
		...
		<p><?php echo sprintf( wp_kses( __( 'Add your desire CSS codes at <a href="%s">Custom CSS</a>', 'cookie' ), array(  'a' => array( 'href' => array() ) ) ), esc_url( admin_url() . 'admin.php?page=cookie-theme-options&tab=29' ) );
		?></p>
  ...
  font-weight: normal;
		...
		<h3 id="instructions"><?php esc_html__('Instructions', 'cookie'); ?></h3>
		<ol>
			<li><?php echo wp_kses(__( 'Generate the Webfont at <a href="https://www.web-font-generator.com/" target="blank">Web Font Generator</a>.', 'cookie' ), array( 'a' => array( 'href' => array() ) ) ); ?></li>
			<li><?php esc_html_e('In case, you\'ve few variations of same font, you should upload it with different name.', 'cookie'); ?></li>
		</ol>
