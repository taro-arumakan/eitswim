...
if( function_exists('custom_widgets_init') === false ):
  function custom_widgets_init() {
    ...
    register_sidebar( array(
      'name'          => esc_html__( 'Schedule Archive用', 'custom' ),
      'id'            => 'sidebar-2',
      'description'   => esc_html__( '右カラム', 'custom' ),
    ...
    register_sidebar( array(
      'name'          => esc_html__( 'Result Archive用', 'custom' ),
      'id'            => 'sidebar-3',
      'description'   => esc_html__( '右カラム', 'custom' ),
      'before_widget' => '<section id="%1$s" class="widget %2$s">',
      'after_widget'  => '</section>',
    ...
    register_sidebar( array(
      'name'          => esc_html__( 'News一覧用', 'custom' ),
      'id'            => 'sidebar-4',
      'description'   => esc_html__( '右カラム', 'custom' ),
      'before_widget' => '<section id="%1$s" class="widget %2$s">',
      'after_widget'  => '</section>',
    ...
    register_sidebar( array(
      'name'          => esc_html__( 'News詳細用', 'custom' ),
      'id'            => 'sidebar-5',
      'description'   => esc_html__( '右カラム', 'custom' ),
      'before_widget' => '<section id="%1$s" class="widget %2$s">',
      'after_widget'  => '</section>',
...
EOS;
     ...
     * sidebar-3
     * News Archive用
     * Class custom_Widget_News_Archive
     */
    class custom_Widget_News_Archive extends WP_Widget {

      function __construct() {
        parent::WP_Widget(false, $name = 'News Archive一覧');
      }
      ...
      function widget( $args, $instance ) {
        ...
        if( $posts ) {
          foreach ( $posts as $post ) {
            setup_postdata( $post );
            $from_year = date('Y', strtotime($post->post_date));
            $years[$from_year] = $from_year;
          }
        }
        ?>
        ...
        <!--widget archive-->
        <div class="widget clearfix widget-archive">
          <h4 class="widget-title">News Archive</h4>
          <ul class="list list-arrow-icons">
            <?php foreach($years as $year):?>
              <li><a href="/news?y=<?php echo $year;?>"><?php echo $year;?></a></li>
            <?php endforeach;?>
          </ul>
        </div><!--end: widget archive-->
    ...
    register_widget('custom_Widget_News_Archive');

    /**
     * sidebar-3
     * Schedule Archive用
     * Class custom_Widget_Schedule_Archive
     */
    class custom_Widget_Schedule_Archive extends WP_Widget {

      function __construct() {
        parent::WP_Widget(false, $name = 'Schedule Archive一覧');
      }
      ...
      function widget( $args, $instance ) {
        ...
        if( $posts ) {
          foreach ( $posts as $post ) {
            setup_postdata( $post );
            $date_from = SCF::get( 'date_from', $post->ID );
        ...
        <!--widget archive-->
        <div class="widget clearfix widget-archive">
          <h4 class="widget-title">Schedule Archive</h4>
          <ul class="list list-arrow-icons">
            <?php foreach($years as $year):?>
              <li><a href="/schedule?y=<?php echo $year;?>"><?php echo $year;?></a></li>
            <?php endforeach;?>
          </ul>
        </div><!--end: widget archive-->
    ...
    register_widget('custom_Widget_Schedule_Archive');

    /**
     * sidebar-2
     * Result Archive用
     * Class custom_Widget_Result_Archive
     */
    class custom_Widget_Result_Archive extends WP_Widget {

      function __construct() {
        parent::WP_Widget(false, $name = 'Result Archive一覧');
      }

      function widget( $args, $instance ) {
        $args = array(
          'post_type' => 'result',    //投稿タイプの指定
          'meta_key' => 'date_from',
          'orderby'  => 'meta_value',
          'posts_per_page' => -1,
          'order' => 'DESC',
        ...
        <!--widget archive-->
        <div class="widget clearfix widget-archive">
          <h4 class="widget-title">Result Archive</h4>
          <ul class="list list-arrow-icons">
            <?php foreach($years as $year):?>
              <li><a href="/result?y=<?php echo $year;?>"><?php echo $year;?></a></li>
            <?php endforeach;?>
          </ul>
        </div><!--end: widget archive-->
        <?php
    ...
    register_widget('custom_Widget_Result_Archive');
