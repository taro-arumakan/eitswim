...
add_filter( 'document_title_parts', function( $title ) {
	...
	switch( true ) {
      ...
      elseif( $luxe['title_top_list'] === 'catch_site' ) {
        $ret = array( 'title' => $title['tagline'], 'tagline' => THK_SITENAME );
        if( isset( $title['page'] ) ) $ret['page'] = $title['page'];
			}
      $top_seo_title = get_theme_mod( 'top_seo_title', '' );
      if(!empty($top_seo_title)) {
        $ret = array( 'title' => $top_seo_title );
      }
			break;
		case is_front_page():
      ...
      $elc_seo_title = get_post_meta($post->ID, 'elc_seo_title', true);
			if(!empty($elc_seo_title)) {
        $ret = array( 'title' => $elc_seo_title );
      }
			break;
		default:
			if( $luxe['title_other'] === 'title' ) {
	...
	if($is_news===true) {
    if($is_news === true) {
      $title = get_option( 'elc_news_page_news_title', 'News' );
      $ret['title'] = $title;
    }
  }
  //子テーマのcustom_document_title_parts()でタイトルを変更できるようにする
  if( function_exists('custom_document_title_parts') === true ):
    $ret = custom_document_title_parts($ret);
  endif;
 ...
 * ヘッダーに gtag 追加
 *---------------------------------------------------------------------------*/
if( function_exists( 'thk_gtag_js' ) === false && is_admin() === false ):
...
<?php if(!empty($html_hreflang)):?>
<link rel="alternate" href="<?php echo esc_url( $canonical_url ); ?>" hreflang="<?php echo $html_hreflang;?>">
<?php endif;?>
<?php
  endif;}
endif;
