...
{
  add_submenu_page('luxe', esc_html__( 'Typekit Fonts', 'luxe' ), esc_html__( 'TypeKit Fonts', 'luxe' ), 'edit_theme_options', 'cookie-typekit-font', 'agni_typekit_render_form' );
  /**
	add_submenu_page('cookie', esc_html__( 'Typekit Fonts', 'cookie' ), esc_html__( 'TypeKit Fonts', 'cookie' ), 'edit_theme_options', 'cookie-typekit-font', 'agni_typekit_render_form' );
   */
}
...
function agni_typekit_render_form()
{
	...
	?>
	<div class="wrap">
		<h1><?php esc_html_e( 'Typekit Fonts', 'cookie' ) ?></h1>
		...
		<form method="post" action="options.php">
			...
			<div class="form-table">
				<h2 class="agni-typekit-text-field-title"><?php esc_html_e( 'Enter Typekit Kit ID', 'cookie' ) ?></h2>
				...
				<p class="typekit-instruction"><?php echo wp_kses(__( 'See the instructions at <a href="#instructions" target="blank">bottom</a>', 'cookie' ), array( 'a' => array( 'href' => array() ) ) ); ?></p>
			</div>
			<p class="agni-typekit-form-submit submit">
				<input type="submit" class="button button-primary button-large" value="<?php esc_attr_e( 'Save Typkit kit ID', 'cookie' ) ?>" />
			</p>
		</form><br/>
	
	<?php if ( $agni_typekit_options['agni_typekit_id'] != '' ) { ?>
		<h3><?php echo esc_html_e( 'Font List :', 'cookie' ); ?></h3>
		<p><?php echo esc_html_e( 'List of font you\'re using on Typekit.', 'cookie' ); ?></p>
		...
		<thead>
			<tr>
				<?php echo '<th>'.esc_html__( 'Font', 'cookie' ).'</th><th>'.esc_html__( 'Font Family', 'cookie' ).'</th><th>'.esc_html__( 'Variations', 'cookie' ).'</th><th>'.esc_html__( 'URL', 'cookie' ).'</th>'; ?>
			</tr>
		</thead>
		<tfoot>
			<tr>
	...
	<h3><?php esc_html_e( 'Font usage :', 'cookie' ); ?></h3>
	<p><?php echo sprintf( wp_kses( __( 'Add your desire CSS codes at <a href="%s">Custom CSS</a>', 'cookie' ), array(  'a' => array( 'href' => array() ) ) ), esc_url( admin_url() . 'admin.php?page=cookie-theme-options&tab=29' ) ); ?></p>
  ...
  font-style: italic;
	...
	<h3 id="instructions"><?php esc_html_e('Instructions', 'cookie'); ?></h3>
	<ol>
		<li><?php echo wp_kses(__( 'Go to <a href="https://typekit.com/" target="blank">typekit.com</a> and register for an account', 'cookie' ), array( 'a' => array( 'href' => array() ) ) ); ?></li>
		<li><?php esc_html_e('Choose a few fonts to add to your account and Publish them', 'cookie'); ?></li>
		<li><?php esc_html_e('Click Kit Editor at the top right and get your Typekit kit ID(at the bottom)', 'cookie'); ?></li>
	</ol>
